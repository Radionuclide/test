...
namespace iba.Processing
{
    internal class FileQuotaCleanup
    {
        ...
        private void Reset()
        {
            m_failureWhilePreviouslyScanning = false;
            m_size = 0;
            m_files.Clear();
            ...
            try
            {
                TraverseFileSystem(DateAndNames);
        ...
        protected virtual void TraverseFileSystem(List<DateAndName> DateAndNames)
        {
        ...
        protected void Log(Logging.Level level, string message, string datfile)
        {
            LogExtraData data = new LogExtraData(datfile, m_task, m_task.ParentConfigurationData);
            if (LogData.Data.Logger != null && LogData.Data.Logger.IsOpen)
            {
        ...
        {
            if (m_failureWhilePreviouslyScanning)
            {
                Reset();
                if (m_failureWhilePreviouslyScanning) return; //fix next time.
            }
            ...
            while (m_size > GetQuota() && m_files.Count > 0)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            string message = String.Format(iba.Properties.Resources.logCleanupSkippedNonIbaDatFile, file);
                            Log(iba.Logging.Level.Info, message, datfile);
                        }
                    }
                    catch (Exception ex)
        ...
        protected virtual bool IsIbaDatfile(string file)
        {
            return true;
        }
