...
namespace iba.Utility
{
	...
	public class Shares : ReadOnlyCollectionBase
	{
		...
		private struct NETRESOURCE
		{
			...
			public int iUsage;
			[MarshalAs(UnmanagedType.LPTStr)] public string sLocalName;
			[MarshalAs(UnmanagedType.LPTStr)] public string sRemoteName;
			[MarshalAs(UnmanagedType.LPTStr)] public string sComment;
			[MarshalAs(UnmanagedType.LPTStr)] public string sProvider;
		}
		...
		protected static void EnumerateSharesNT(string server, Shares shares, ref bool accessDenied)
		{
			int level = 2;
		...
		/// <param name="server">The server name</param>
		/// <param name="shares">The Shares</param>
		protected static void EnumerateShares9x(string server, Shares shares, ref bool accessDenied)
		{
			int level = 50;
			int nRet = 0;
		...
		/// <param name="server">The server name</param>
		/// <param name="shares">The Shares</param>
		protected static void EnumerateShares(string server, Shares shares, ref bool accessDenied)
		{
		...
		/// </summary>
		/// <param name="UNCcomputer">Remote computer</param>
		/// <param name="bForce">Force disconnect even if files are open on remote share</param>
		/// <returns></returns>
		public static int DisconnectFromComputer(string UNCcomputer, bool bForce) 
		{  
			string comp = UNCcomputer;
			return WNetCancelConnection2(comp, 0, bForce);
		}
		...
		public static string PathToUnc(string fileName, bool convertLocalPaths) 
		{
			...
			switch (nRet) 
			{
				...
				case ERROR_NOT_CONNECTED:
					//Local file-name
                    if(convertLocalPaths)
                    {
                        Shares shi = LocalShares;
                        if (null != shi)
                        {
		...
		private static Shares _local = null;

		/// <summary>
		/// Return the local shares
		/// </summary>
		public static Shares LocalShares 
		{
			get 
			{
				if (null == _local) 
		...
		/// <param name="server"></param>
		/// <returns></returns>
		public static Shares GetShares(string server)
		{
			return new Shares(server);
		}
